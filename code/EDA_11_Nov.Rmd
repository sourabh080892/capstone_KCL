...
drug_cases <- cs_1[cs_1$hearing_event_classification == 'Drug Court', ] %>%
                select(c(file_number))

drug_cases = rbind(drug_case_desc, drug_cases)
drug_cases <- drug_cases[!duplicated(drug_cases), ]
...
rm(cs_1, cs_2, case_stage_by_file, drug_case_desc)
...
### Case is drug case - Yes/No
dfa_by_category$drug_case <- ifelse(is.na(match(dfa_by_category$file_number, drug_cases$file_number)),"No", "Yes")
...
ggplot(dispo_by_category,aes(x=dispo_time_filing)) +
  geom_histogram()  + 
    xlim(c(-200,1200)) + facet_grid(~drug_case) + ggtitle("Drug Cases")

# drug_court
summaryBy(dispo_time_filing ~ drug_case, data = dispo_by_category, 
          FUN = list(mean, max, min, median), na.rm=TRUE)
#-----------------------------------------------------
```
...
# drug_court
summaryBy(dispo_time_arraign ~ drug_case, data = dispo_by_category, 
          FUN = list(mean, max, min, median), na.rm=TRUE)
#-----------------------------------------------------
```
...
ggplot(dispo_by_category, aes(x=total_dfa, y=dispo_time_filing)) +
  geom_point(stat = "summary", fun.y = "mean") +
  facet_grid(~drug_case) + ggtitle("Drug Cases")
```
