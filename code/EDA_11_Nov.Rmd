...
#----------------------  Received new Data for D24 and D13  -----------------#
D13_case_event_key <- read_excel("../Data/Data for SU Part 4 - cleaned criminal history and updated hearing key.xlsx", sheet = "CaseEvent Key") %>% clean_names()
D24_crime_hist <- read_excel("../Data/Data for SU Part 4 - cleaned criminal history and updated hearing key.xlsx", sheet = "SU CrimHist") %>% clean_names()
```
...
```{r}
### ----------  Now we have case_stage from DAVID, No mutation required now  ---------- ###
### ----------  Commentiing the code  ------------------------------------------------- ###
...
D23_dispo_key <- D23_dispo_key %>% 
  ...
  mutate(
    ...
    dispo_type = case_when(
      str_detect(tolower(type), "trial")     ~ "trial",
      str_detect(tolower(type), "plea")      ~ "plea",
      str_detect(tolower(type), "dismissal") ~ "dismissal",
      TRUE ~ "dismissal"
    )
  ) %>% 
  mutate_at(c("verdict", "dispo_type"), factor)

# delete records where ranking is 5
D23_dispo_key <- D23_dispo_key %>%
   filter(ranking != 5)
...
# criminal history sort by recent crime per defendant id
crime_hist_by_def <- D24_crime_hist %>%
                        group_by(defendant_id) %>% 
...
2.	 Critical stage of a case is case setting  (Arraignment, case setting, before case setting, after case setting) -> `case_stage_by_file`

3.	Last Arraignment Date -> `last_arr_date_by_file`
...
# Drug cases definition 1
drug_case_desc <- represent_charge_by_file
drug_case_desc$drug_case <- ifelse(grepl("Violation of the Uniform Controlled Substances Act", represent_charge_by_file$description), "yes", "no")
# filter records where drug_case is 'yes'
drug_case_desc <- drug_case_desc[drug_case_desc$drug_case == 'yes', ] %>%
                    select(c(file_number))

# join 'represent_charge_by_file' and 'D23_dispo_key' to get type of disposition (Trail, Plea, Dismissal)
represent_charge_by_file <- left_join(represent_charge_by_file, D23_dispo_key, by = c("disposition_code"))
...
represent_charge_by_file <- represent_charge_by_file %>%
                              ...
                              slice(1) %>%
                              ungroup()  %>%
                              select(-c(offense_start_date, offense_end_date, filing_date, disposition_date, disposition_code,
                                        charge_enhanced, description.x, description.y, ranking, type, verdict))
...
# filter records where case is drug_Case
drug_court <- cs_1[cs_1$hearing_event_classification == 'Drug Court', ] %>%
                select(c(file_number))

# filter records where case has competency hearing
compt_cases <- cs_1[cs_1$hearing_event_classification == 'Competency', ]

# last arraignment date for each file number
last_arr_date_by_file <- cs_1[cs_1$hearing_event_classification == 'Arraignment', ] %>%
...
colnames(last_arr_date_by_file)[colnames(last_arr_date_by_file) == 'hearing_event_classification'] <- 'arraignment'

# first case setting date for each file number
first_cs_date_by_file <- cs_1[cs_1$hearing_event_classification == 'Case Setting', ] %>%
                        select(-c(defendant_id, event_code, event_enter_date, defendant_event_status)) %>%
                        group_by(file_number) %>%
...
colnames(first_cs_date_by_file)[colnames(first_cs_date_by_file) == 'hearing_event_classification'] <- 'case_setting'
...
dfa_file_cases <- cs_1[cs_1$event_code == 'DEFFTA', ]
...
dfa_file_cases <- dfa_file_cases %>%
                    ...
                    arrange(file_number, event_enter_date) %>%
                    slice(1) %>%
                    select(c(file_number, hearing_event_classification, total_dfa)) %>%
                    ungroup()
...
rm(cs_1, cs_2, case_stage_by_file)
...
```{r}
# We need DFA rates of per file_number by 
# Age, Age-group, Gender, Venue      -- in D11_filed_cases dataset
# charge class, seriousness, violent -- in represent_charge_by_file
# Criminal History                   -- in D24_crime_hist / crime_hist_by_file
# Critical stage of case, total dfa  -- in dfa_file_cases
# Custody Status                     -- events_with_missing_value
# Drug case(Yes/No)                  -- in drug_cases
# Competency Case(Yes/No)            -- in compt_cases
...
dfa_by_category <- merge(x = dfa_by_category, y = dfa_file_cases, by = "file_number", all.x=TRUE)
  # fill total_dfa value nas with 0
dfa_by_category$total_dfa[is.na(dfa_by_category$total_dfa)] <- 0
  # new column in "dfa_by_category" matching with "dfa_file_cases" to check if filed case has DFA - yes/no
dfa_by_category$dfa <- ifelse(is.na(match(dfa_by_category$file_number, dfa_file_cases$file_number)),"No", "Yes")
...
### Case charge is drug charge - Yes/No
dfa_by_category$drug_charge <- ifelse(is.na(match(dfa_by_category$file_number, drug_case_desc$file_number)),"No", "Yes")

### Case went into drug court - Yes/No
dfa_by_category$drug_court <- ifelse(is.na(match(dfa_by_category$file_number, drug_court$file_number)),"No", "Yes")

### Case has atleast one competency hearing - Yes/No
dfa_by_category$compt_case <- ifelse(is.na(match(dfa_by_category$file_number, compt_cases$file_number)),"No", "Yes")

### Club GM, M, I, U as one class - GM
dfa_by_category <- dfa_by_category %>%
                    mutate(class= recode(class, "c('GM', 'M', 'U', 'I')='GM'"))

### new dummy variable for age <=18
dfa_by_category$le_18 <- ifelse(dfa_by_category$age <= 18, "Yes", "No")

### new dummy variable for age >=60
dfa_by_category$ge_60 <- ifelse(dfa_by_category$age >= 60, "Yes", "No")
...
dispo_by_category <- merge(x = dispo_time_dispo_rank, y = dfa_by_category, by = "file_number", all.x=TRUE) %>%  
      mutate(serious_cat = ifelse(seriousness <= 5, '<=5', 
                         ...
                         select(-c(filing_date, last_arr_date, case_types, charge_code.y,
                                   seriousness))

# # export dfa_by_category to csv
write.csv(dispo_by_category, '../calculated_data/dispo_by_category.csv')
```
...
ggplot(dispo_by_category,aes(x=dispo_time_filing)) +
  geom_histogram()  + 
    xlim(c(-200,1200)) + facet_grid(~drug_charge) + ggtitle("Drug Charge")

# drug_court
summaryBy(dispo_time_filing ~ drug_charge, data = dispo_by_category, 
          FUN = list(mean, max, min, median), na.rm=TRUE)
#-----------------------------------------------------
...
ggplot(dispo_by_category,aes(x=dispo_time_filing)) +
  geom_histogram()  + 
    xlim(c(-200,1200)) + facet_grid(~drug_court) + ggtitle("Drug Court")
...
# drug_court
summaryBy(dispo_time_arraign ~ drug_charge, data = dispo_by_category, 
          FUN = list(mean, max, min, median), na.rm=TRUE)
#-----------------------------------------------------
...
ggplot(dispo_by_category, aes(x=total_dfa, y=dispo_time_filing)) +
  geom_point(stat = "summary", fun.y = "mean") +
  facet_grid(~drug_charge) + ggtitle("Drug Charge")

ggplot(dispo_by_category, aes(x=total_dfa, y=dispo_time_filing)) +
  geom_point(stat = "summary", fun.y = "mean") +
  facet_grid(~drug_court) + ggtitle("Drug Court")
```
