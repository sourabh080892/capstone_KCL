...
D21_charge <- D21_charge %>% 
  mutate_at(c("referral_date", "offense_start_date", "offense_end_date", "filing_date"), mdy) %>% 
  mutate(
    ...
    filing_gap = interval(offense_end_date, filing_date) %/% months(1)
  ) %>% 
...
D23_dispo_key <- D23_dispo_key %>% 
  mutate(
    type = factor(type),
    disposition_code = fct_recode(type, Other = "Dismissal", Other = "Decline or Other/Error")
  )
D24_crime_hist <- D24_crime_hist %>% 
  mutate(
    ...
    felony_misd = if_else(endsWith(conviction_level, 'M'), "misdemeanour", "felony"),
  )
